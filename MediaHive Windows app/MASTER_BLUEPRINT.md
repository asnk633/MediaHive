...
| :--- | :--- | :--- |
| 2026-07-07 | **Secret Purging & Repository Hygiene**: Deleted Tauri updater private key file (`mediahive.key`) from desktop folder. Updated subfolder `.gitignore` to exclude `*.key` and `*.log` files to prevent security leaks in future commits. Run `git rm --cached` to untrack existing private keys and backups. | AI Agent |

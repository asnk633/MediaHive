        ...
        const init = async () => {
            ...
            try {
                ...
                setUser({
                    ...
                    role: (finalProfile?.role || session.user.app_metadata?.role || 'member').toLowerCase() === 'guest' ? 'member' : (finalProfile?.role || session.user.app_metadata?.role || 'member'),
                ...
                const fetchProfileWithRetry = async () => {
                    ...
                    try {
                        ...
                        setUser({
                            ...
                            email: finalProfile?.email || session.user.email || '',
                            name: finalProfile?.name || finalProfile?.full_name || 'User',
                            role: (finalProfile?.role || 'member').toLowerCase() === 'guest' ? 'member' : (finalProfile?.role || 'member'),
    ...
    const refreshUser = async () => {
        ...
        if (session?.user) {
            ...
            setUser({
                ...
                role: (profile?.role || 'member').toLowerCase() === 'guest' ? 'member' : (profile?.role || 'member'),

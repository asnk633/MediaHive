...
/**
 * Deterministic test-time Firebase mock
 *
 * When USE_REAL_FIREBASE is not "true", we inject a tiny in-page init script
 * which sets window.__FIREBASE_READY__ and other debug flags so runtime
 * assertions in the suite succeed reliably in CI/local dev.
 *
 * To run tests against real Firebase run:
...
test.describe('6. WebView Device Behavior Tests', () => {
  for (const ua of WEBVIEW_UAS) {
    test(`${ua.name} - Complete WebView Validation`, async ({ page }) => {
      ...
      page.on('console', (msg: any) => {
        const txt = msg.text();
        console.log(`[BROWSER][${ua.name}][${msg.type()}] ${txt}`);
        logs.push({
          type: msg.type(),
          text: txt
        });

...
export function WorkspacesClient({ initialWorkspaces }: { initialWorkspaces: (Institution & { type: 'institution' | 'department' })[] }) {
    const [workspaces, setWorkspaces] = useState<(Institution & { type: 'institution' | 'department' })[]>(initialWorkspaces);
    // Since we start with initial server-rendered workspaces, we don't need initial loading state
    const [loading, setLoading] = useState(false);
    const [search, setSearch] = useState('');
    
    // Select first visible active workspace automatically on init
    const firstActive = initialWorkspaces.find(w => w.status !== 'archived');
    const [selectedId, setSelectedId] = useState<string | null>(firstActive ? firstActive.id : null);

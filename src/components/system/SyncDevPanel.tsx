...
export function SyncDevPanel() {
  ...
  const row = (label: string, value: React.ReactNode, highlight?: string) => (
    <div className="flex items-start justify-between gap-4 py-1.5 border-b border-white/5 last:border-0">
      ...
      <span className={`text-xs font-mono text-right break-all ${highlight ?? 'text-white/80'}`}>{value ?? '—'}</span>
    </div>
  );
  ...
  return (
    <div className="fixed bottom-16 right-4 z-[1000] w-80 bg-black/90 backdrop-blur border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-2">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 bg-white/5 border-b border-white/10">
        <span className="text-xs font-bold text-white/70 uppercase tracking-wider">⚙ Sync Dev Panel</span>
        <button onClick={() => setVisible(false)} className="text-white/50 hover:text-white/70 text-xs">✕</button>
      </div>

      {/* Live metrics */}
      <div className="px-4 py-3">
        <p className="text-[10px] text-white/50 uppercase tracking-widest mb-2">Live</p>
        {metrics ? (
          <>
            ...
            {row('Total Retries', metrics.retries, metrics.retries > 0 ? 'text-yellow-300' : 'text-white/80')}
      ...
      {persisted && (
        <div className="px-4 pb-3 border-t border-white/5 mt-1">
          <p className="text-[10px] text-white/50 uppercase tracking-widest mb-2 mt-2">Last Telemetry Snapshot</p>
      ...
      <div className="px-4 pb-3 text-[10px] text-white/20 text-center">

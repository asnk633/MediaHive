...
export function MemberWelcomeModal({ onOpenGuide }: MemberWelcomeModalProps) {
    const { user } = useAuth();
    ...
    useEffect(() => {
        ...
        // 3. 'hasSeenMemberWelcome' must be false (or missing)
        if (user && user.role === 'member') {
            // Versioned key allows re-showing onboarding if content changes in future
            const hasSeen = localStorage.getItem("hasSeenMemberWelcome-v1");
            if (!hasSeen) {

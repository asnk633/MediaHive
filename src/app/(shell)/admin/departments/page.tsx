...
export default function DepartmentManagementPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(-1); // -1 for new department
    
    try {
      const url = editingId ? `/api/departments?id=${editingId}` : '/api/departments';
      ...
      alert("Failed to save department: " + (e as Error).message);
    } finally {
      setSaving(null);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      alert("Failed to delete department: " + (e as Error).message);
    } finally {
      setSaving(null);
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen app-body-padding px-4 pb-24 max-w-4xl mx-auto">
      <header className="mb-8 pt-6">
        <h1 className="text-3xl font-display font-bold text-white mb-2">Department Management</h1>
        <p className="text-[var(--color-text-secondary)]">Manage departments in your organization.</p>
      </header>

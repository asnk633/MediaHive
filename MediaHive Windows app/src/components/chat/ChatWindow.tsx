...
export default function ChatWindow({ 
  ...
  // Fetch Message Stream directly using client-side Supabase
  ...
  useEffect(() => {
    ...
    // 1. Fetch initial message stream
    fetchMessages();
    
    // 2. Setup real-time postgres changes subscription on chat_messages table
    ...
    // 3. Keep fallback polling interval (longer, e.g. 10s) to guarantee consistency in background
  ...
  // Listen for clear chat events via global eventBus
  useEffect(() => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Update room metadata in client-side
      await supabase
        .from('chat_rooms')
        .update({
  ...
  const uploadFile = async (file: File, mode: 'gallery' | 'file' | 'voice' = 'gallery', durationSeconds?: number) => {
    ...
    try {
      // Direct POST to hosted Vercel backend upload API
      ...
      // Insert message direct to Supabase
      ...
      else if (mediaType === 'voice') previewText = 'Voice note';
      else if (textPayload) previewText = textPayload;
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative overflow-hidden bg-transparent">
      ...
      <div className="h-20 border-b border-white/[0.05] flex items-center justify-between px-6 bg-black/15 shrink-0 z-10">
        ...
        {/* Header Left: Click to toggle GroupInfoSidebar */}
        <button 
          onClick={() => setInfoOpen(!infoOpen)}
          className="flex items-center gap-3.5 text-left focus:outline-none cursor-pointer group animate-in fade-in slide-in-from-left duration-300"
        >
          <div className={`h-10 w-10 rounded-xl bg-gradient-to-br ${headerIconGradient} flex items-center justify-center text-white shrink-0 font-bold text-sm tracking-wider shadow-md shadow-black/25 group-hover:scale-105 transition-transform duration-300 overflow-hidden`}>
            {isImageIcon ? (
              <img 
                src={getDriveImageUrl(room.icon_url, undefined, true)} 
                alt={initials} 
        ...
        {loading ? (
          ...
          <div className="text-center text-[#a1a1aa] text-xs py-24 flex flex-col items-center justify-center gap-3 relative z-10 animate-in fade-in zoom-in duration-300">
            ...
            <div>
              ...
              <p className="text-[11px] font-light text-[#a1a1aa]">{searchQuery ? 'Try searching for other keywords.' : 'Say hello or share a document to begin.'}</p>
            </div>
          </div>
        ) : (
                ...
                <React.Fragment key={msg.id}>
                  {showDateHeader && (
                    <div className="flex justify-center my-4 select-none">
                      <div className="bg-white/5 border border-white/10 px-3 py-1 rounded-full text-[10px] font-bold text-white/55 tracking-wider uppercase backdrop-blur-sm shadow-sm">
                    ...
                    <div className={`flex flex-col gap-1 ${isMe ? 'items-end' : 'items-start'}`}>
                      {showHeader && (
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="text-[11px] font-semibold text-white/80 tracking-wide">{senderName}</span>
                        </div>
                      )}
...
{/* Bubble + Actions Wrapper */}
                      ...
                      >
                        {/* Bubble */}
                        <div 
                          className={`py-2 shadow-lg flex flex-col transition-all duration-300 ${
                            isMe 
                              ? 'bg-white/[0.07] border border-white/[0.06] hover:bg-white/[0.1] hover:border-indigo-500/30 hover:scale-[1.02] hover:shadow-[0_0_20px_rgba(99,102,241,0.15)] text-white pl-3.5 pr-7' 
                              : 'bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.06] hover:border-teal-500/30 hover:scale-[1.02] hover:shadow-[0_0_20px_rgba(20,184,166,0.15)] text-white pl-7 pr-3.5'
                          }`}
                          style={{
                        ...
                        >
                          {msg.is_deleted ? (
                            <p className="text-xs italic text-white/40 leading-relaxed font-light select-none py-1">
                                ...
                                <div className="flex flex-col gap-2 py-1 min-w-[200px]">
                                  ...
                                  <div className="flex justify-end gap-2">
                                    <button 
                                      ...
                                      className="px-2.5 py-1 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] text-white/80 transition-colors cursor-pointer"
                                    >
                                      Cancel
                                    </button>
                                    <button 
                              ...
                              {msg.media_url && (
                                <div className={msg.text ? "mt-2.5 border-t border-white/10 pt-2.5" : ""}>
                                  {msg.media_type === 'image' ? (
                                    <div className="relative overflow-hidden rounded-xl border border-white/10 bg-black/30 group/img max-w-sm">
                                      <img 
                                        src={getDriveImageUrl(msg.media_url, msg.drive_file_id, true)} 
                                        alt="attachment" 
                                        className="rounded-none max-h-60 w-full object-cover transition-transform duration-300 group-hover/img:scale-105" 
                                      />
                                      <a 
                                    ...
                                    <div className="relative overflow-hidden rounded-xl border border-white/10 bg-black/40 shadow-xl max-w-[280px] sm:max-w-sm">
                                      <video 
                        ...
                        {!msg.is_deleted && (msg.sender_id === currentUser.id || participants.find(p => p.user_id === currentUser.id)?.role?.toLowerCase() === 'manager' || participants.find(p => p.user_id === currentUser.id)?.role?.toLowerCase() === 'creator') && (
                          ...
                          >
                            <div
                              ...
                              className="cursor-pointer hover:text-white text-white/55 p-1 rounded-lg hover:bg-white/5 transition-colors flex items-center justify-center shrink-0"
                              title="Actions"
                            >
          ...
          <motion.button
            ...
            animate={{ 
              ...
              boxShadow: [
                ...
                "0 20px 40px -8px rgba(99, 102, 241, 0.8)",
                "0 20px 40px -8px rgba(99, 102, 241, 0.4)"
              ]
            }}

...
test.describe('Downloads/Files Feature', () => {
  ...
  test('Upload file to Google Drive', async ({ page }) => {
    ...
    // Mock API requests for file uploads to prevent actual uploads if the app allows it
    await page.route('**/api/upload*', async route => {
        await route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({ success: true, fileId: 'mock-drive-id-123' })
        });
    });
    ...
    // The app has an Upload button (lucide-react UploadCloud usually or text "Upload")
    // Wait for and click the main upload button to open the modal
    const mainUploadBtn = page.locator('button:has-text("Upload"), button [aria-label="Upload"], button:has(.lucide-upload-cloud)');

    // We should check if an upload button is visible
    if (await mainUploadBtn.count() > 0 && await mainUploadBtn.first().isVisible()) {
        await mainUploadBtn.first().click();
        ...
        // Submit upload in the modal
        const submitBtn = modal.locator('button:has-text("Upload")');

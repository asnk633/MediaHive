    ...
    <ModalBase open={open} onClose={onClose} panelClass="fixed inset-x-0 bottom-0 z-[70] mx-auto max-w-xl rounded-t-2xl bg-[#102220] p-4">
      ...
      <div className="space-y-5">
        <label className="block">
          ...
          <input value={title} onChange={(e) => setTitle(e.target.value)} className="h-12 w-full rounded-xl border border-[#00b39d]/30 bg-foreground/5 px-4" />
        </label>
        <label className="block">
          <p className="pb-2">Message Body</p>
          <textarea value={body} onChange={(e) => setBody(e.target.value)} className="min-h-28 w-full rounded-xl border border-[#00b39d]/30 bg-foreground/5 p-4" />
        </label>
        <div className="pt-2">

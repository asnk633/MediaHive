...
>(({ className, children, ...props }, ref) => (
    <SelectPrimitive.Trigger
        ref={ref}
        className={cn(
            "flex h-10 w-full items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-slate-200 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 focus:ring-offset-slate-950 disabled:cursor-not-allowed disabled:opacity-50",
            "backdrop-blur-md shadow-sm transition-all duration-300 hover:bg-white/[0.06]",
            className
        )}
        {...props}
    >
...
>(({ className, children, position = "popper", ...props }, ref) => (
    <SelectPrimitive.Portal>
        <SelectPrimitive.Content
            ref={ref}
            className={cn(
                "relative z-50 min-w-[8rem] overflow-hidden rounded-xl border border-white/10 bg-slate-950/90 text-slate-200 shadow-elevated animate-in fade-in-80",
                "backdrop-blur-xl",
                className
            )}
            position={position}

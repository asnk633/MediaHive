    ...
    return (
      <div className="p-8 space-y-8 animate-in fade-in duration-500">
        ...
        <div className="space-y-4">
            <Skeleton className="h-20 rounded-3xl bg-foreground/5" />
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    <Skeleton className="h-64 rounded-3xl bg-foreground/5" />
                    <Skeleton className="h-96 rounded-3xl bg-foreground/5" />
                </div>
                <div className="space-y-8">
                    <Skeleton className="h-48 rounded-3xl bg-foreground/5" />
                    <Skeleton className="h-96 rounded-3xl bg-foreground/5" />
                </div>
            </div>
        </div>
  ...
  if (!data) {
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center text-center p-8">
        ...
        <h2 className="text-2xl font-bold text-foreground mb-2">Production Not Found</h2>
        <Button 
          variant="outline" 
          onClick={() => router.back()}
          className="border-foreground/10 hover:bg-foreground/5 text-foreground"
        >
    ...
    <div className="p-8 pb-20 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div className="flex items-center gap-5 flex-1">
          <button 
            onClick={() => router.back()}
            className="p-3 rounded-2xl bg-foreground/[0.03] border border-foreground/5 text-foreground/80 hover:text-foreground hover:bg-foreground/[0.08] transition-all"
          >
            <ArrowLeft size={20} />
          </button>
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-1">
              ...
              <div 
                className="flex items-center gap-1.5 cursor-pointer hover:text-foreground transition-colors text-foreground/80 group"
            ...
            {isEditing ? (
                <input 
                    ...
                    className="w-full bg-foreground/[0.04] border border-foreground/10 rounded-xl px-4 py-2 text-2xl font-black text-foreground outline-none focus:border-blue-500/50 transition-all font-sans tracking-tighter"
                />
            ) : (
                <h1 className="text-4xl font-black text-foreground tracking-tighter">{event.title}</h1>
            )}
          </div>
        </div>
            ...
            {isEditing ? (
                <>
                    <Button 
                        ...
                        disabled={saving}
                        variant="ghost"
                        className="text-foreground/80 hover:text-foreground hover:bg-foreground/5 rounded-xl px-6 font-bold"
                    >
                        Cancel
                    </Button>
                    <Button 
                        onClick={handleSave}
                        disabled={saving}
                        className="bg-emerald-600 hover:bg-emerald-500 text-foreground border-emerald-400/20 rounded-xl px-6 shadow-lg shadow-emerald-500/20 font-bold"
                    >
                ...
                <Button 
                    ...
                    className="bg-foreground/5 hover:bg-foreground/10 text-foreground border-foreground/10 rounded-xl px-6 font-bold"
                >
                    Edit Protocol
                </Button>
            )}
      ...
      <div className="mb-12">
        <ReactiveCard className="px-8 py-6 bg-foreground/[0.02] border-foreground/5 rounded-[32px]">
            ...
            <div className="relative px-2 py-4">
                <div className="absolute top-[36px] left-0 right-0 h-[2px] bg-foreground/5 z-0" />
                        ...
                        return (
                            <div key={stage.id} className="flex flex-col items-center group cursor-default">
                                <div className={cn(
                                    ...
                                    status === 'completed' ? "bg-emerald-500 border-emerald-400 text-foreground" :
                                    status === 'active' ? "bg-blue-600 border-blue-400 text-foreground shadow-lg shadow-blue-500/20" :
                                    "bg-slate-900 border-foreground/10 text-foreground/80"
                                )}>
                                ...
                                <div className="mt-3.5 flex flex-col items-center h-10">
                                    <span className={cn(
                                        ...
                                        status === 'locked' ? "text-foreground/80" : "text-foreground"
                                    )}>
                                        {stage.label}
                                    </span>
                                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">        {/* Left Column: Details & Stage-based Tasks */}
        <div className="lg:col-span-2 space-y-12">
          ...
          {/* Main Info Card */}
          <ReactiveCard className="p-6 bg-foreground/[0.01] border-foreground/5 rounded-[32px] relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-8">
                    <div>
                        <h3 className="text-[10px] font-black uppercase tracking-widest text-foreground/70 mb-4 flex items-center gap-2">
                        ...
                        {isEditing ? (
                            ...
                            <div className="space-y-3 mb-4">
                                <p className="text-foreground font-bold flex items-center gap-3">
                                    <span className="text-blue-400">{format(startDate, 'EEEE, MMMM do')}</span>
                                </p>
                                <p className="text-3xl text-foreground/90 font-black tracking-tight mb-2">
                    ...
                    <div className="pt-2">
                        ...
                        {isEditing ? (
                            <input 
                                ...
                                className="w-full bg-foreground/[0.04] border border-foreground/10 rounded-xl px-4 py-2 text-sm font-bold text-foreground outline-none focus:border-blue-500/50"
                                placeholder="Studio / On Location"
                            />
                        ) : (
                            <p className="text-xl text-foreground font-black tracking-tight">{event.location || 'Studio A / Remote'}</p>
                        )}
                    </div>
                </div>
                    ...
                    <div className="pt-2">
                        ...
                        {isEditing ? (
                            <textarea 
                                ...
                                className="w-full bg-foreground/[0.04] border border-foreground/10 rounded-xl px-4 py-3 text-sm font-medium text-foreground/80 outline-none focus:border-blue-500/50 min-h-[120px] resize-none leading-relaxed"
                                placeholder="Detail the production requirements..."
                            />
                        ) : (
                            <p className="text-sm text-foreground/80 leading-relaxed font-medium" style={{ lineHeight: '1.5' }}>
          ...
          <div className="space-y-8">
            <h2 className="text-2xl font-black text-foreground flex items-center gap-3 px-2 mb-6">
                        ...
                        <div key={stage.id} className={cn(
                            ...
                            <div 
                                ...
                                className={cn(
                                    ...
                                    status === 'active' ? "bg-foreground/[0.03] border-foreground/10" :
                            ...
                            >
                                <div className="flex items-center gap-5">
                                    ...
                                    <div className="space-y-1">
                                        <h3 className="text-sm font-black uppercase tracking-widest text-foreground">{stage.label}</h3>
                                ...
                                <div className="flex items-center gap-6">
                                    {status === 'locked' && (
                                        <span className="text-[9px] font-black uppercase tracking-widest text-foreground/80 bg-foreground/5 px-3 py-1.5 rounded-lg border border-foreground/5">
                                            Awaiting Previous Stage
                                        </span>
                                    )}
                                    {isExpanded ? <ChevronUp size={20} className="text-foreground/80" /> : <ChevronDown size={20} className="text-foreground/80" />}
                                </div>
                            </div>

                            {isExpanded && (
                                <div className="mt-6 ml-5 pl-10 border-l-2 border-foreground/5 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                    {stageTasks.length > 0 ? stageTasks.map((task) => (
                                        <ReactiveCard 
                                            key={task.id} 
                                            className={cn(
                                                "p-5 bg-foreground/[0.02] border-foreground/5 hover:bg-foreground/[0.04] transition-all rounded-[24px] group",
                                                ...
                                                <div className="flex items-center gap-5">
                                                    <button 
                                                        ...
                                                        className={cn(
                                                            ...
                                                            task.status === 'done' ? "bg-emerald-500 border-emerald-500 text-foreground" : "border-foreground/20"
                                                        )}
                                                    >
                                                    ...
                                                    <div className="space-y-1">
                                                        <p className={cn(
                                                            ...
                                                            task.status === 'done' ? "text-foreground/70 line-through" : "text-foreground"
                                                        )}>
                                                            {task.title}
                                                        </p>
                                                        <p className="text-[10px] text-foreground/70 font-black uppercase tracking-widest px-1">
                                                            {task.priority} Priority
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[9px] font-black uppercase tracking-widest text-foreground/70 group-hover:text-foreground/70 transition-colors bg-foreground/5 px-2 py-1 rounded">
                                                        #{task.id.slice(0, 4)}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="p-10 text-center bg-foreground/[0.01] border border-dashed border-foreground/10 rounded-3xl">
                                            <p className="text-[10px] text-foreground/80 font-black uppercase tracking-widest">No milestones defined for this stage.</p>
                                        </div>
                                    )}
                                </div>
                            )}
        ...
        <div className="space-y-12">
            {/* Crew Card */}
            <div className="space-y-6">
                <h2 className="text-xl font-black text-foreground flex items-center gap-3 px-4">
                    <Users size={20} className="text-blue-400" />
                    Assigned Crew
                </h2>
                <ReactiveCard className="bg-foreground/[0.01] border-foreground/5 overflow-hidden rounded-[32px]">
                    <div className="divide-y divide-white/5">
                        {crew.length > 0 ? crew.map((member) => (
                            <div key={member.id} className="p-6 flex items-center justify-between hover:bg-foreground/[0.03] transition-all group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-sm font-bold text-foreground group-hover:text-blue-400 transition-colors">{member.profile?.full_name}</p>
                                        <p className="text-[10px] text-foreground/70 font-black uppercase tracking-widest">{member.role || 'Staff'}</p>
                                    </div>
                                </div>
                                <div className="w-9 h-9 rounded-xl bg-foreground/5 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all border border-foreground/5">
                                    <ChevronRight size={16} className="text-foreground/80" />
                                </div>
                            </div>
                        )) : (
                            <div className="p-12 text-center bg-foreground/[0.01]">
                                <p className="text-[10px] font-black uppercase tracking-widest text-foreground/80">Awaiting Assignment</p>
                            </div>
                        )}
                    </div>
                </ReactiveCard>
                ...
                <ReactiveCard className="bg-foreground/[0.01] border-foreground/5 overflow-hidden rounded-[32px]">
                    <div className="divide-y divide-white/5">
                        {equipment.length > 0 ? equipment.map((item) => (
                            <div key={item.id} className="p-6 flex items-center justify-between hover:bg-foreground/[0.03] transition-all group">
                                <div className="flex items-center gap-5">
                                    <div className="w-12 h-12 rounded-xl bg-foreground/5 flex items-center justify-center border border-foreground/5 overflow-hidden group-hover:border-blue-500/20 transition-all shadow-lg shadow-black/20">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-sm font-bold text-foreground max-w-[140px] truncate">{item.inventory?.name}</p>
                                        <p className="text-[10px] text-foreground/70 font-black uppercase tracking-widest">{item.inventory?.category || 'Inventory'}</p>
                                    </div>
                                </div>
                            ...
                            <div className="p-12 text-center bg-foreground/[0.01]">
                                <p className="text-[10px] font-black uppercase tracking-widest text-foreground/80">No Gear Reserved</p>
                            </div>
                        )}
                    </div>
                </ReactiveCard>

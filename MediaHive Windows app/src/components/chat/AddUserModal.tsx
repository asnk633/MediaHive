...
export default function AddUserModal({ room, currentUser, allUsers, trigger }: any) {
  ...
  const handleAddUser = async () => {
    ...
    setLoading(true);
    try {
      // 1. Fetch profiles for selected user IDs
      ...
      if (usersToAdd.length > 0) {
        ...
        if (managers && managers.length > 0) {
          ...
          const notifs = managers.map((m: any) => ({
            ...
            body: `${namesList} ${usersToAdd.length > 1 ? 'were' : 'was'} added to "${room.name || 'group'}" by ${currentUser.fullName}. Remove them if this is incorrect.`,
            channel: "ui",
            created_at: now,
          }));

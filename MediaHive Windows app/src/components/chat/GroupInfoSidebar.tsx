...
export default function GroupInfoSidebar({ 
  ...
  const handleDeleteRoom = async () => {
    ...
    try {
      await Promise.all([
        supabase.from('chat_participants').delete().eq('room_id', room.id),
        ...
        <div className="border-t border-white/[0.04] pt-4.5">
          <div className="flex items-center justify-between text-left mb-3">
            ...
            <AddUserModal 
              ...
              allUsers={allUsers}
              trigger={
                <button className="text-[10px] font-bold text-indigo-400 hover:text-indigo-300 flex items-center gap-1 hover:underline cursor-pointer bg-transparent border-none">
                  Add User
                </button>
              }
            />
          ...
          <div className="border-t border-white/[0.04] pt-5 mt-6 shrink-0 space-y-3">
            <Dialog open={isClearingChat} onOpenChange={setIsClearingChat}>
              <DialogTrigger asChild>
                <button className="w-full py-2.5 px-4 rounded-xl border border-amber-500/20 bg-amber-500/5 hover:bg-amber-500/10 text-amber-400 hover:text-amber-300 text-xs font-semibold tracking-wide transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer bg-transparent">
                  <Trash2 className="h-3.5 w-3.5" />
                  Clear Chat Messages
                </button>
              </DialogTrigger>
              <DialogContent className="bg-zinc-950 border border-amber-500/30 text-white shadow-2xl rounded-2xl max-w-sm">
            ...
            <Dialog open={isDeleting} onOpenChange={setIsDeleting}>
              <DialogTrigger asChild>
                <button className="w-full py-2.5 px-4 rounded-xl border border-rose-500/20 bg-rose-500/5 hover:bg-rose-500/10 text-rose-400 hover:text-rose-300 text-xs font-semibold tracking-wide transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer bg-transparent">
                  <Trash2 className="h-3.5 w-3.5" />
                  Delete Group
                </button>
              </DialogTrigger>
              <DialogContent className="bg-zinc-950 border border-rose-500/30 text-white shadow-2xl rounded-2xl max-w-sm">
                <DialogHeader className="text-center">

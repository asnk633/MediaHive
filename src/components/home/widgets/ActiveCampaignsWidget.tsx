...
export const ActiveCampaignsWidget = () => {
        ...
        return (
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="h-40 rounded-2xl bg-foreground/[0.02] border border-foreground/5 animate-pulse" />
                    ))}
                </div>
            </div>
        );
    ...
    if (error) {
        return (
            <div
                className="group relative min-h-[220px] rounded-2xl border border-foreground/5 bg-foreground/[0.01] flex flex-col items-center justify-center text-center p-8 backdrop-blur-md hover:bg-foreground/[0.02] hover:border-foreground/10 transition-all duration-300 hover:-translate-y-[2px] animate-in fade-in duration-700"
            >
                {/* 3px Accent Strip (Safe Fallback) */}
                <div className="absolute top-0 left-0 w-full h-[3px] bg-foreground/10 opacity-30 group-hover:opacity-100 transition-opacity duration-500" />

                <div className="w-12 h-12 rounded-full bg-foreground/5 flex items-center justify-center mb-5 text-foreground/80 transition-colors group-hover:text-foreground/80">
                    <FolderPlus size={24} />
                </div>

                <h3 className="text-sm font-bold text-foreground/80 uppercase tracking-widest mb-1 group-hover:text-foreground/80 transition-colors">
                    No active campaigns right now
                </h3>
                <p className="text-[10px] text-foreground/80 mb-6 max-w-[240px] leading-relaxed group-hover:text-foreground/70 transition-colors">
                    Check back shortly or create a new campaign to get started.
                </p>

                <button
                    onClick={() => window.location.reload()}
                    className="px-6 py-2 text-[10px] font-bold text-foreground/80 bg-foreground/5 hover:bg-foreground/10 rounded-xl transition-all border border-foreground/10 uppercase tracking-[0.2em] active:scale-95 shadow-sm"
                >
                    Retry Sync
                </button>
            </div>
    ...
    return (
        <div className="space-y-6">
            ...
            {campaigns.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 px-6 border border-dashed border-foreground/10 rounded-2xl bg-foreground/[0.01] animate-in fade-in duration-700">
                    <div className="w-12 h-12 rounded-full bg-foreground/5 flex items-center justify-center mb-4 text-foreground/80">
                        <FolderPlus size={24} />
                    </div>
                    <h3 className="text-sm font-bold text-foreground/80 uppercase tracking-widest">No active campaigns right now</h3>
                    <p className="text-[10px] text-foreground/80 mt-1 text-center">New initiatives will surface here once created.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {campaigns.map(campaign => {
                        ...
                        return (
                            <div
                                ...
                                className="group relative flex flex-col justify-between p-5 rounded-2xl border border-foreground/5 bg-foreground/[0.01] backdrop-blur-md hover:bg-foreground/[0.02] hover:border-blue-500/20 transition-all duration-300 cursor-pointer overflow-hidden active:scale-[0.98] shadow-sm hover:shadow-blue-500/5 text-left hover:-translate-y-[2px]"
                            >

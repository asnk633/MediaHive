...
function CallbackHandler() {
  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      logDiagnostic(`Browser Callback: Loaded. code=${code ? "present" : "absent"}, isTauri=${isTauri}, searchParams=${searchParams.toString()}`, "browser");
      if (code) {
        try {
          logDiagnostic("[Callback] Exchanging PKCE code for session in browser...", "browser");
          const { data, error } = await supabase.auth.exchangeCodeForSession(code);
          if (error) throw error;
          
          if (data?.session) {
            const session = data.session;
            logDiagnostic("[Callback] Session established in browser successfully.", "browser");
            if (isTauri) {
              ...
              logDiagnostic(`[Callback] Forwarding session tokens to Tauri via deep link: ${tauriUrl}`, "browser");
              setStatus("success");
              window.location.href = tauriUrl;
              logDiagnostic("[Callback] window.location.href redirect called.", "browser");
            } else {
              logDiagnostic("[Callback] Non-Tauri flow. Redirecting browser to home page.", "browser");
              router.replace("/");
            }
            return;
          } else {
            logDiagnostic("[Callback] Warning: code exchange finished but no session returned.", "browser");
          }
        } catch (err: any) {
          logDiagnostic(`[Callback] Code exchange error: ${err.message || err}`, "browser");
      ...
      if (isTauri) {
        logDiagnostic("[Callback] No code parameter found. Polling for session as fallback...", "browser");
        let session = null;
        for (let i = 0; i < 20; i++) {
        ...
        if (session) {
          const tauriUrl = `mediahive://login#access_token=${session.access_token}&refresh_token=${session.refresh_token}&expires_in=${session.expires_in}&token_type=bearer`;
          logDiagnostic(`[Callback] Fallback session found. Redirecting browser to: ${tauriUrl}`, "browser");
          setStatus("success");
          window.location.href = tauriUrl;
        } else {
          logDiagnostic("[Callback] Fallback session polling failed.", "browser");
          setStatus("error");
          router.replace("/auth/error?error=session_failed");
        }
      } else {
        logDiagnostic("[Callback] Non-Tauri callback with no code. Redirecting browser to home page.", "browser");
        router.replace("/");
      }
    };

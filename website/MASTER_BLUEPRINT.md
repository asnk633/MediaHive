...
*   **Re-Sync Engine**: Once network status transitions to online, `syncEngine.ts` triggers a sync run, flushes mutations, and sends updates to Next.js API where Last-Write-Wins (LWW) is used to silently resolve edits. If the server overwrites any offline client edit, a sync notification is saved in IndexedDB (Dexie `syncNotifications` table) to display an inline banner inside the task details view.
...
| Date | Change | Author |
| :--- | :--- | :--- |
| 2026-06-27 | **Verified Web E2E Test Suite (56/56 Pass):** Re-built Next.js client with `NEXT_PUBLIC_E2E_TESTING=true` bypass flag, resolved port collisions, and ran Chromium tests on port 3005 with 100% success. | AI Agent |
| 2026-06-27 | **Marketing Site UI Fixes:** Fixed feature card text contrast (0.58 -> 0.7, passes WCAG AA). Removed broken waitlist modal (fake backend), replaced "Join Waitlist" buttons with "GitHub" links, removed duplicate "Download App". Cleaned up unused CSS. | AI Agent |
| 2026-06-17 | **Playwright E2E Triage & Stabilization:** Systematically diagnosed and fixed 151 E2E test failures using pattern-first approach. Fixed authentication bypasses in TasksNewClient (missing isGuest and  wait), and resolved hydration race conditions across critical journeys. | AI Agent |
| 2026-06-17 | **Progressive Web App Upgrade:** Upgraded the Next.js SaaS platform to a fully installable PWA using `@ducanh2912/next-pwa`. Configured `next.config.mjs` with explicit `networkOnly` runtime caching for `/api/` routes to prevent stale live data. Generated dynamic manifest via App Router `metadata` export in `layout.tsx`, extracting legacy service worker cleanup into a separate `PWALegacyCleanup` client component. Debounced the `OfflineBanner` by 1500ms to prevent UI flicker during momentary network drops. Added iOS Safari manifest keys and `apple-touch-icon`. Verified 100% build health and unit test success. | AI Agent |
| 2026-06-17 | **Kanban Board Accessibility:** Made the drag-and-drop Kanban Board fully accessible to keyboard and screen reader users. Refactored `TaskKanbanView.tsx` to include a custom `SpaceKeyboardSensor` that activates dragging only on `Spacebar` (avoiding conflict with `Enter`). Built `kanbanKeyboardCoordinates` custom coordinate getter to map Arrow keys (`ArrowLeft`/`ArrowRight`/`ArrowUp`/`ArrowDown`) to adjacent columns with horizontal smooth scrolling support. Added `tabIndex={0}` and `role="listitem"` to task cards, `role="list"` to task list wrappers, and reactive column headers description `aria-label`. Added concise custom screen reader announcements inside `DndContext` for grab/move/drop status updates. Updated Playwright E2E tests `kanban-dnd.spec.ts` to simulate keyboard navigation, verifying the full end-to-end accessible drag-and-drop flow across Chromium, Firefox, and WebKit (6/6 tests pass). | AI Agent |
| 2026-06-17 | **Hardened Offline Sync LWW & Jest Test Suite:** Added support for header-based idempotency keys (`Idempotency-Key`) in PUT `/api/tasks/[id]` API endpoint. Updated Jest unit test mocks and conflict assertions in `api-tasks-id.test.ts` to ensure 100% Jest test suite success. | AI Agent |
| 2026-06-16 | **Silent Auto-Merge & LWW Sync Conflict Resolution:** Migrated offline sync conflict resolution from manual diff selection sheet (`SyncConflictSheet`/`SyncConflictModal`) to a seamless Last-Write-Wins (LWW) model on the backend Tasks API (`PUT /api/tasks/[id]`), comparing `client_timestamp` vs `updated_at`. When server version wins, non-conflicting fields are auto-merged, and `updated_by_server: true` is returned. Implemented local sync notification stores (`syncNotifications` Dexie table on Web) to track overwritten client changes. Added non-blocking inline warning banners at the top of Task Details pages on Web (`TaskDetailModalV2`) with persistent dismissal. Excluded `scratch` directory from web TypeScript compilation in `tsconfig.json`. | AI Agent |

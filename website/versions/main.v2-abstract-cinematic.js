...
// ==========================================================================
// 3.75. Desk Awakening — Cinematic 3D Hero Background
...
function initDeskAwakening() {
  ...
  // Ambient lighting (deep dark indigo void)
  const hemiLight = new THREE.HemisphereLight(0x180f30, 0x05030a, 0.85);
  scene.add(hemiLight);

  // Key Studio Spot Light casting soft shadows
  ...
  // Rim Light (cool blue from laptop direction)
  const rimLight = new THREE.PointLight(0x00f0ff, 0.1, 10);
  rimLight.position.set(-1, 1.2, -0.6);
  scene.add(rimLight);

  // Warm fill light (rich purple/violet)
  const warmFill = new THREE.PointLight(0x6c3ef4, 1.5, 12);
  warmFill.position.set(3.5, 2.5, 4.5);
  scene.add(warmFill);

  // Reflective Floor Grid
  const floorGrid = new THREE.GridHelper(30, 30, 0x4f3987, 0x181230);
  floorGrid.position.y = -0.49;
  scene.add(floorGrid);

  const floorPlane = new THREE.Mesh(
    new THREE.PlaneGeometry(50, 50),
    new THREE.MeshStandardMaterial({ color: 0x060412, roughness: 0.22, metalness: 0.88 })
  );
  ...
  // ── 1. Interactive GPU Particles (Seesaw Style) ───────────────────────────
  ...
  for (let i = 0; i < particleCount * 3; i += 3) {
    ...
    positions[i + 1] = Math.random() * 4.5 - 0.4;
    positions[i + 2] = (Math.random() - 0.5) * 7.5;
  ...
  // ── 2. Floating Translucent Glass Bento Tiles (Media Nodes) ───────────────
  const cardGeo = new THREE.BoxGeometry(0.72, 1.08, 0.04);
  ...
  function createMediaCard(colorHex, innerMesh) {
    const cardGroup = new THREE.Group();

    // The Glass Panel
    const plate = new THREE.Mesh(cardGeo, glassMat);
    plate.castShadow = false; // Disable to prevent blocky black shadow bug on glass
    plate.receiveShadow = true;
    cardGroup.add(plate);

    // Glowing Neon Edge Line
    const borderMat = new THREE.LineBasicMaterial({ color: colorHex, linewidth: 2 });
    const border = new THREE.LineSegments(edges, borderMat);
    cardGroup.add(border);

    // Inner Mesh representing media
    if (innerMesh) {
      innerMesh.position.z = 0.04;
      cardGroup.add(innerMesh);
    }

    // Inside card glow pointlight
    const cardLight = new THREE.PointLight(colorHex, 0.7, 3.5);
    cardLight.position.set(0, 0, 0.08);
    cardGroup.add(cardLight);

    return { group: cardGroup, light: cardLight };
  }

  // Create inner nodes
  const photoNode = new THREE.Mesh(
    new THREE.SphereGeometry(0.15, 16, 16),
    new THREE.MeshStandardMaterial({ color: 0x00f0ff, roughness: 0.15, metalness: 0.8, emissive: 0x00a0ff, emissiveIntensity: 0.6 })
  );
  const videoNode = new THREE.Mesh(
    new THREE.ConeGeometry(0.14, 0.26, 4),
    new THREE.MeshStandardMaterial({ color: 0xd500f9, roughness: 0.1, metalness: 0.85, emissive: 0x9c27b0, emissiveIntensity: 0.6 })
  );
  videoNode.rotation.x = Math.PI / 2;

  const audioNode = new THREE.Mesh(
    new THREE.TorusGeometry(0.14, 0.035, 8, 24),
    new THREE.MeshStandardMaterial({ color: 0xffab00, roughness: 0.1, metalness: 0.95, emissive: 0xff6d00, emissiveIntensity: 0.5 })
  );

  const docNode = new THREE.Group();
  for (let j = 0; j < 3; j++) {
    const bar = new THREE.Mesh(
      new THREE.BoxGeometry(0.22, 0.035, 0.015),
      new THREE.MeshStandardMaterial({ color: 0x00e676, roughness: 0.25, metalness: 0.7, emissive: 0x00c853, emissiveIntensity: 0.5 })
    );
    bar.position.y = 0.10 - j * 0.10;
    docNode.add(bar);
  }

  const card1 = createMediaCard(0x00f0ff, photoNode);
  const card2 = createMediaCard(0xd500f9, videoNode);
  const card3 = createMediaCard(0xffab00, audioNode);
  const card4 = createMediaCard(0x00e676, docNode);

  // Set card base offsets (relative to center)
  const CARD_OFFSETS = {
    c1: { x: -2.0, y: 1.2, z: 0.8, rx: 0.1, ry: 0.4, rz: -0.05 },
    c2: { x: 2.0, y: 1.5, z: 0.5, rx: -0.2, ry: -0.5, rz: 0.1 },
    c3: { x: -1.6, y: 1.7, z: -1.2, rx: 0.3, ry: 0.6, rz: -0.15 },
    c4: { x: 1.8, y: 0.8, z: 1.6, rx: -0.15, ry: -0.3, rz: 0.08 }
  };

  sceneGroup.add(card1.group, card2.group, card3.group, card4.group);

  // ── 3. Custom Volumetric Screen Ray Shader ────────────────────────────────
  const rayGeo = new THREE.CylinderGeometry(1.5, 0.3, 3.5, 32, 1, true);
  rayGeo.translate(0, 1.75, 0); // pivot at base
  ...
  const rayMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        float vertFade = pow(1.0 - vUv.y, 2.0); // smooth vertical fade out
        float radFade = sin(vUv.x * 3.14159);   // radial edge feathering
        float pulse = 0.8 + 0.2 * sin(uTime * 4.5);
        float alpha = vertFade * radFade * uOpacity * pulse * 0.45;
        
        vec3 col = vec3(0.0, 0.85, 1.0); // cool cyan hologram color
  ...
  const screenRay = new THREE.Mesh(rayGeo, rayMat);
  screenRay.position.set(0, 0.08, -0.22);
  screenRay.rotation.x = -Math.PI * 0.16; // sweep forward slightly
  sceneGroup.add(screenRay);

  // ── 4. Laptop GLTF Loading & Setup ────────────────────────────────────────
  ...
  deskLoader.load('/models/macbook_pro_14_inch_M5.glb', (gltf) => {
    ...
    model.position.set(-center.x, -bbox.min.y - 0.5, -center.z);
    ...
    model.traverse((child) => {
      ...
      if (isScreen) {
        if (desktopTexture) {
          screenMat.map = desktopTexture;
          screenMat.emissiveMap = desktopTexture;
        }
    ...
    if (!laptopLid) laptopLid = model;
    laptopLid.rotation.x = Math.PI * 0.85; // closed rotation
    laptopGroup.add(model);
  });

  // ── 5. Post Processing Composer with Quality Cascades ─────────────────────
  let composer = null;
  if (!isMobile) {
  ...
  const animState = {
    lidRotationX: Math.PI * 0.85,
    screenEmissive: 0.0,
    screenRayOpacity: 0.0,
    cardDistance: 1.6, // tiles start dispersed
    cardOpacity: 0.0,  // start hidden
    particleFlow: 0.15,
    ambientLight: 0.1
  };
  ...
  function updateVisuals() {
    ...
    rayMat.uniforms.uOpacity.value = animState.screenRayOpacity;
    particleMat.uniforms.uFlowIntensity.value = animState.particleFlow;
    rimLight.intensity = animState.ambientLight * 2.2;

    const d = animState.cardDistance;
    const op = animState.cardOpacity;

    card1.group.position.set(CARD_OFFSETS.c1.x * d, CARD_OFFSETS.c1.y * d, CARD_OFFSETS.c1.z * d);
    card2.group.position.set(CARD_OFFSETS.c2.x * d, CARD_OFFSETS.c2.y * d, CARD_OFFSETS.c2.z * d);
    card3.group.position.set(CARD_OFFSETS.c3.x * d, CARD_OFFSETS.c3.y * d, CARD_OFFSETS.c3.z * d);
    card4.group.position.set(CARD_OFFSETS.c4.x * d, CARD_OFFSETS.c4.y * d, CARD_OFFSETS.c4.z * d);

    card1.group.scale.setScalar(op);
    card2.group.scale.setScalar(op);
    card3.group.scale.setScalar(op);
    card4.group.scale.setScalar(op);
  }

  function resetScene() {
    animState.lidRotationX = Math.PI * 0.85;
    animState.screenEmissive = 0.0;
    animState.screenRayOpacity = 0.0;
    animState.cardDistance = 1.6;
    animState.cardOpacity = 0.0;
    animState.particleFlow = 0.15;
    animState.ambientLight = 0.1;
    updateVisuals();
  }

  // ── 7. GSAP Main Loop Timeline ────────────────────────────────────────────
  const tl = gsap.timeline({ repeat: -1, repeatDelay: 0.5 });

  // Phase 1 (0-1.5s): Calm hover, cards drift in and fade visible
  tl.to(animState, {
    cardDistance: 1.0,
    cardOpacity: 1.0,
    particleFlow: 0.22,
  ...
  // Phase 2 (4.2s): Laptop lid sweeps open
  tl.to(animState, {
    lidRotationX: -Math.PI * 0.14,
    ambientLight: 1.0,
    duration: 2.4,
    ease: 'power3.inOut',
    onUpdate: updateVisuals
  ...
  // Phase 3 (4.8s): Screen emits glow and volumetric holographic rays rise
  tl.to(animState, {
    screenEmissive: 1.25,
    screenRayOpacity: 1.0,
    particleFlow: 0.45,
    duration: 1.8,
    ease: 'power2.out',
    onUpdate: updateVisuals
  ...
  // Phase 4 (7-10.5s): Hold state, pulsing rays

  // Phase 5 (10.6-12s): Outro fade via Vignette, snap reset, crossfade back
  tl.to(vignette, {
    opacity: 1,
    duration: 0.6,
    ease: 'power2.in'
  ...
  // ── 8. Dampened Mouse Parallax Tilt (Design Spell) ────────────────────────
  ...
  // ── 10. Intersection Observer (Off-screen render freeze) ─────────────────
  let active = true;
  const ioObs = new IntersectionObserver(([e]) => {
    active = e.isIntersecting;
    if (active && !rafId) startRenderLoop();
  }, { threshold: 0 });
  ...
  function startRenderLoop() {
    (function loop() {
      ...
      // Slow organic rotation of glass tiles
      card1.group.rotation.y = CARD_OFFSETS.c1.ry + Math.sin(time * 0.3) * 0.08;
      card1.group.rotation.x = CARD_OFFSETS.c1.rx + Math.cos(time * 0.2) * 0.06;
      card1.group.position.y += Math.sin(time * 0.8) * 0.0015;

      card2.group.rotation.y = CARD_OFFSETS.c2.ry + Math.sin(time * 0.25) * 0.07;
      card2.group.rotation.x = CARD_OFFSETS.c2.rx + Math.cos(time * 0.3) * 0.05;
      card2.group.position.y += Math.cos(time * 0.75) * 0.0015;

      card3.group.rotation.y = CARD_OFFSETS.c3.ry + Math.sin(time * 0.32) * 0.09;
      card3.group.rotation.x = CARD_OFFSETS.c3.rx + Math.cos(time * 0.22) * 0.06;
      card3.group.position.y += Math.sin(time * 0.65) * 0.0015;

      card4.group.rotation.y = CARD_OFFSETS.c4.ry + Math.sin(time * 0.28) * 0.08;
      card4.group.rotation.x = CARD_OFFSETS.c4.rx + Math.cos(time * 0.26) * 0.06;
      card4.group.position.y += Math.cos(time * 0.85) * 0.0015;

      // Pulse volumetric rays slightly
      screenRay.scale.set(1 + Math.sin(time * 3.0) * 0.02, 1, 1 + Math.sin(time * 3.0) * 0.02);
...
// ==========================================================================
// 5.5  Navbar Scroll Reveal — appears after 2500px, fades on reverse
// ==========================================================================
(function initNavbarReveal() {
  const nav = document.querySelector('.main-nav');
  if (!nav) return;

  // Variant 1 (default): Fade + slide down from -8px — elegant entrance
  // Variant 2: Fade + scale from 0.96
  // Variant 3: Pure fade only

  const SCROLL_SHOW = 2500; // px — show navbar after this
  const SCROLL_HIDE = 2300; // px — hide again if scroll goes below this (hysteresis)
  ...
  // Lenis is initialised slightly after this IIFE — defer one tick

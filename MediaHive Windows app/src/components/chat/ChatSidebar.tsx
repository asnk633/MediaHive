...
export default function ChatSidebar({ currentUser, rooms, activeRoom, unreadCounts = {}, onSelectRoom, onRoomCreated }: any) {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      // 2. Fetch Media & IT team members to auto-add them
      ...
      const insertedUserIds = new Set([currentUser.id]);
      ...
      onRoomCreated({ id: roomId, name: newRoomName.trim(), createdBy: currentUser.id, isMediaTeamOnly: false, created_at: now });
      setIsCreating(false);
      setNewRoomName('');
    } catch (err) {
      console.error(err);

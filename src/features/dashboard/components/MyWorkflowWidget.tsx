...
export const MyWorkflowWidget = ({ tasks, userId }: MyWorkflowWidgetProps) => {
    ...
    const myTasks = tasks.filter(task => {
        ...
        if (Array.isArray(task.assignedTo)) {
            return task.assignedTo.some(u => (u as any).uid === userId);
        }
        return false;
    });
    ...
    myTasks.forEach(task => {
        const stage = (task as any).smartMetadata?.inferredStage;
    ...
    const StageCard = ({ label, subtitle, count, icon: Icon, colorClass, stageKey }: any) => (
        <button
            ...
            disabled={count === 0}
            className={`
                flex flex-col items-center justify-center p-4 rounded-[18px] border transition-all duration-300 w-full relative group
                ${count > 0

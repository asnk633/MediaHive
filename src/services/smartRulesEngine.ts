...
export class SmartRulesEngine {
     ...
     * 100 = Overdue or Due Today + Urgent Priority
     * 0 = No due date, low priority
     */
    ...
    /**
     * Calculate Task Load per User per Stage
     * Returns: Map<UserId, { [Stage]: number }>
     */
    static getUserStageLoad(tasks: Task[]): Record<string, Record<string, number>> {
        ...
        tasks.forEach(task => {
            ...
            assignees.forEach(user => {
                ...
                if (loadMap[user.uid][stage] !== undefined) {
                    loadMap[user.uid][stage]++;
                }
            });
        });
